...
5.* --Comparing the amount of forest area lost between 1990 and 2016, to which country's total area in 2016 is it closest to?
...
REGIONAL
*/
1.* --Create a table that shows the Regions and their percent forest area from 1990 to 2016.
...
3.* --Which region had the HIGHEST percent forest in 2016, and which had the LOWEST?
...
5.* --Which region had the HIGHEST percent forest in 1990, and which had the LOWEST?
SELECT region, MAX(percent_forest) AS max
FROM regionals
WHERE year = 1990
GROUP BY 1
...
6.* --Which regions of the world DECREASED in forest area from 1990 to 2016?
WITH pf1990 AS (
